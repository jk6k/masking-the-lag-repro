...
- G1 release artifact: status `pass`, manuscript `pass` with `1070` lines, red-team `pass` across `4` lenses, external status `explicitly_abandoned`, public-repro `pass` with validation `pass`.
