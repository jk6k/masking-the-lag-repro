...
- G1 release artifact: status `pass`, manuscript `pass` with `1283` lines, red-team `pass` across `4` lenses, external status `not_available_in_local_execution`, public-repro `pass` with validation `pass`.
